...
class Serial_Main_Backend(QMainWindow):
    ...
    def init_nodecon(self,isnodedown=False,nodedown=0):
        node_connections = []
    ...
    def append_adjMtx_conn(self):
        len_adjmx = 0
        for i in range(0,self.numberofnodes):
            for a in range (0,self.numberofnodes):
                self.nodesadj.append(self.adj_arr[i][a])
                len_adjmx =+ 1
        return len_adjmx
    def update_graph(self,isnodedown=False,nodedown=0):
